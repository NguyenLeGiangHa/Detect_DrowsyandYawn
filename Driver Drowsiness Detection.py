...
video_source = 0  # Use 0 for the default camera, or provide a video file path
...
def writeEyes(leftEye, rightEye, frame):
    if len(leftEye) > 0:
        # Get bounding box coordinates for left eye
        x1 = min(leftEye[:, 0])
        y1 = min(leftEye[:, 1])
        x2 = max(leftEye[:, 0])
        y2 = max(leftEye[:, 1])
        ...
        padding = 10
        eye_region = frame[max(0, y1-padding):min(frame.shape[0], y2+padding), 
                          max(0, x1-padding):min(frame.shape[1], x2+padding)]
        
        if eye_region.size > 0:  # Check if region is not empty
            cv2.imwrite('left-eye.jpg', eye_region)
    ...
    if len(rightEye) > 0:
        ...
        if eye_region.size > 0:  # Check if region is not empty
            cv2.imwrite('right-eye.jpg', eye_region)
...
while True:
    ...
    for rect in rects:
        ...
        # Add error checking for eye regions
